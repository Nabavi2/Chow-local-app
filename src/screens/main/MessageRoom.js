...
export const MessageRoomScreen = ({navigation}) => {
  ...
  const messageTerritories = useSelector(
    state => state.order.messageTerritories,
  );
  ...
  const getMoreMessages = () => {
    ...
    fetchAPI(
      `/messages?territory=${territory.tid}&user=${customer_id}&size=10&page=${
        messagePage + 1
      }&app=seller`,
      {
        method: 'GET',
  ...
  useEffect(() => {
    setLoading(true);
    fetchAPI(
      `/messages?territory=${territory.tid}&user=${customer_id}&size=10&page=0&app=seller`,
      {
        method: 'GET',
        headers: {
  ...
  const updateNewMessage = useCallback(() => {
    ...
    if (lastMessageID > 0) {
      fetchAPI(
        `/messages/last_activity?territory=${territory.tid}&user=${customer_id}&last_message_id=${lastMessageID}&app=seller`,
        {
          method: 'GET',
          headers: {
            authorization: `Bearer ${token}`,
  ...
  useEffect(() => {
    navigation.setParams({
      ...
      territoryAddress:
        territory.warehouse_address_city +
        ' ' +
        territory.warehouse_address_province,
    });
  }, [profile]);
    ...
    <Screen
      ...
      stickyBottom={
        <View
          style={
            Platform.OS === 'ios'
              ? styles.input_message_ios
              : styles.input_message
          }>
          <Input
            type="textarea"
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              ...
              if (userinfo) formData.append('first_name', userinfo.firstName);
              if (userinfo) formData.append('last_name', userinfo.lastName);
              if (userinfo) formData.append('email', userinfo.email);
              ...
              fetchAPI(`/message/create`, {
                ...
                .then(res => {
                  ...
                  const msg = [
                    {
                      by: 'territory',
                      date_created: res.data.message_date,
                      date_opened: '',
                      is_new: false,
      ...
      <View>
        {messageList && messageList.length > 0 && (
          <ScrollView
            ...
            style={{
              maxHeight:
                windowHeight - Theme.header.height - 100 + bottomMargin,
            }}
            onScroll={event => {
              if (
