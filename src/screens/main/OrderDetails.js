...
export const OrderDetailsScreen = ({navigation}) => {
  ...
  const submitDelivery = useMemo(
    () => navigation.getParam('requestDelivery'),
    [],
  );
  ...
  const orderIndex = useMemo(
    () =>
      navigation.getParam('index') != undefined
  ...
  useEffect(() => {
    ...
    if (
      orderDetail &&
      -parseInt(getTimeLeft(orderDetail.prep_time_date)) > 0 &&
      (orderDetail.status_name == 'Processing' ||
        ((orderDetail.delivery_type == 'pickup' ||
          orderDetail.delivery_request_by == 'territory') &&
          orderDetail.status_name == 'Accepted'))
    ) {
      ...
      timer = setInterval(() => {
        if (orderDetail.delivery_type == 'deliver') {
          ...
          if (-parseInt(getTimeLeft(orderDetail.prep_time_date)) > 0) {
            setDriverArriveIn(
              -parseInt(getTimeLeft(orderDetail.prep_time_date)),
            );
          } else {
            setDriverArriveIn(0);
          }
  ...
  useEffect(() => {
    ...
    if (
      orderDetail &&
      15 - getTimeLeft(orderDetail.submit_date) > 0 &&
      orderDetail.status_name == 'New'
    ) {
      ...
      orderDetail &&
      orderDetail.status_name == 'New' &&
      orderDetail.is_pre_order == false
    ) {
      setLoading(true);
          ...
          message: (
            <>
              ...
              <AppText
                ...
                {accepted_data.app_driver_name} has accepted your delivery
                request
              </AppText>
              <AppText
                style={{
                ...
                {accepted_data.show_collect_from_driver == true
                  ? 'DRIVER WILL PAY YOU ' + accepted_data.collect_from_driver
                  : accepted_data.show_pay_to_driver == true &&
                    'PAY TO DRIVER ' + accepted_data.pay_to_driver}
              </AppText>
              <Button
                type="white"
                ...
                onClick={() => {
                  Linking.canOpenURL(
                    `tel:${accepted_data.app_driver_phone}`,
                  ).then(supported => {
                    if (supported) {
                ...
                <AppText
                  ...
                  CALL {accepted_data.app_driver_first_name}
                </AppText>
              </Button>
              <Button
                type="white"
                ...
                onClick={() => {
                  dispatch(
                    setTerritory(JSON.parse(accepted_data.app_territory)),
                  );
          ...
          message: (
            <>
              ...
              <AppText
                ...
                Your delivery request has been submitted. Please wait for a
                delivery person to accept this request.
              </AppText>
              <Button
                type="white"
                fullWidth
    ...
    fetchAPI('/order/details', {
      ...
      .then(res => {
        ...
        RNFetchBlob.config({fileCache: true})
          .fetch(
            'GET',
            printAsText == true ? res.data.txt_url : res.data.image_url,
          )
        ...
        if (res.data.is_pre_order == false) {
          /// only past Orders item's status have to change from new to changed status
          var temp = pastOrders;
          if (orderIndex != null) {
            temp[orderIndex].is_new = false;
            if (
              navigation.getParam('orderStatus') != undefined &&
  ...
  const orderAcceptByPickup = useCallback(
        ...
        <View style={styles.container}>
          {orderDetail.is_pre_order == false && (
            <AppText style={styles.orderstatus}>
              ORDER STATUS:
              <AppText style={styles.orderstatusBold}>
                {' '}
          ...
          {orderDetail.is_pre_order == true && (
            <View
              ...
              <AppText
                ...
                numberOfLines={1}>
                PRE-ORDER {getTimeAway(orderDetail.pre_order_date)}
              </AppText>
            </View>
          )}
          ...
          <View
            ...
            <View style={{flexDirection: 'row', flex: 1, padding: 10}}>
              ...
              <View style={{marginLeft: 15, flex: 2}}>
                <View>
                  <AppText
                    style={{fontWeight: 'bold', paddingRight: 0, marginTop: 3}}
                    numberOfLines={1}>
                ...
                <View style={{marginTop: 3}}>
                  <AppText
                    style={{fontSize: 12, color: '#777', height: 16}}
                    numberOfLine={1}>
                    {orderDetail.is_pre_order == false
                      ? getTimeAgo(orderDetail.submit_date)
                      : getTimeAway(orderDetail.pre_order_date)}{' '}
                  </AppText>
                </View>
              </View>
          ...
          {orderDetail.user.is_guest == false &&
            (orderDetail.users_first_order == true ? (
              <View
                style={{
              ...
              <View
                ...
                <AppText
                  ...
                  numberOfLines={1}>
                  This user has placed {orderDetail.total_orders} orders
                </AppText>
              </View>
            ))}

          {orderDetail.pay_type == 2 && orderDetail.is_paid != true && (
            <View
              style={{
              ...
              <AppText
                ...
                numberOfLines={1}>
                Please Collect Payment of {orderDetail.currency_icon}
                {(+orderDetail.total_amount).toFixed(2)}
              </AppText>
            </View>
          )}

          {orderDetail.request_delivery == true &&
            orderDetail.is_pre_order == false && (
              <View
                style={
                ...
                <AppText
                  ...
                  numberOfLines={1}>
                  {orderDetail.request_delivery_is_accepted == true
                    ? orderDetail.show_pay_to_driver == true
                      ? 'Pay the driver ' + orderDetail.pay_to_driver
                      : orderDetail.show_collect_from_driver == true
                      ? 'The driver will pay you ' +
                        orderDetail.collect_from_driver
                      : 'Delivery Request Accepted'
                    : 'Delivery Requested'}
                </AppText>
                {oneMinPassed == true &&
                orderDetail.request_delivery_is_accepted == false ? (
                  <TouchableOpacity
                    onPress={() => {
                      requestCancel();
                    }}
                    style={{
                      backgroundColor: '#eb011b',
                      height: 50,
                      marginTop: -10,
                      marginRight: -10,
                      width: 70,
          ...
          {orderDetail.status_name == 'New' &&
            (preorder != 'preorder' || orderDetail.is_pre_order == false) && (
                ...
                <Button
                  ...
                  onClick={() => {
                    if (orderDetail.delivery_type == 'pickup') {
                      ...
                      NavigationService.navigate('OrderAccept', {
                        orderId: orderId,
                      });
                    }
                  }}>
            ...
            orderDetail.is_pre_order == false && (
              <View style={{flexDirection: 'row', marginHorizontal: 15}}>
                <Button
                  ...
                  onClick={() => {
                    NavigationService.navigate('OrderReject', {
          ...
          <DashedLine
            styleContainer={{marginTop: 20, marginBottom: 10, borderRadius: 1}}
          />
          {orderDetail.pay_type == 2 &&
            orderDetail.is_paid != true &&
            orderDetail.status_name == 'Accepted' && (
              <View style={{flexDirection: 'row', marginHorizontal: 15}}>
                <Button
                        ...
                        message: (
                          <>
                            ...
                            <AppText
                              ...
                              {(+orderDetail.total_amount).toFixed(2)} has been
                              received
                            </AppText>
                            <Button
                              type="white"
          ...
          {orderDetail.is_pre_order == false && (
            <View style={{flexDirection: 'row', marginHorizontal: 15}}>
                      ...
                      if (orderDetail.request_delivery_is_accepted == true) {
                        dispatch(
                          showNotification({
                            type: 'error',
                            message:
                              "Once you have requested delivery, you can't update the order status anymore. Our driver will take it from here",
                            buttonText: 'Go Back',
                          }),
                        );
                      } else {
              ...
              {selectedPrinterIdentifier != null &&
                base64Image != '' &&
                orderDetail.status_name != 'New' &&
                orderDetail.status_name != 'Rejected' && (
                  <Button
                    type="bordered-dark"
                    style={{
                      flex: 2,
                    ...
                    onClick={async () => {
                      if (printAsText == true) {
                        ...
                        try {
                          var builder =
                            new StarXpandCommand.StarXpandCommandBuilder();
                          builder.addDocument(
          ...
          {orderDetail.is_pre_order == true && (
            <View style={{flexDirection: 'row', marginHorizontal: 15}}>
              <Button
                ...
                onClick={() => {
                  NavigationService.navigate('ETADetails', {
                    pre_orderDetail: orderDetail,
                  });
                }}>
                ETA DETAILS
              </Button>
          ...
          <View style={styles.orderInfo}>
            <View style={{flexDirection: 'row'}}>
              <View style={{flex: 1}}>
                <AppText style={styles.field}>Payment</AppText>
              ...
              {orderDetail.promo_code_amount > 0 && (
                <View style={{flex: 1}}>
                  ...
                  {orderDetail.auto_discount_amount ==
                  orderDetail.promo_code_amount ? (
                    <View>
            ...
            <View style={styles.part}>
              {orderDetail.delivery_type == 'pickup' ? (
                <View style={styles.part}>
                  ...
                  <AppText style={styles.value_pickup}>Pick up</AppText>
                </View>
              ) : (
                <View>
                  <View style={styles.part}>
                  ...
                  <View style={styles.part}>
                    ...
                    <AppText style={styles.value_address}>
                      ...
                      {orderDetail.address_type != '' &&
                        ', ' + orderDetail.address_type}{' '}
                      {orderDetail.address_apartment_nr != '' &&
                        ', ' + orderDetail.address_apartment_nr}
                    </AppText>
                  </View>
                </View>
          ...
          <FlatList
            ...
            renderItem={({item}) => (
              <CartItem orderDetail={orderDetail} product={item} />
            )}
            ListFooterComponent={() => (
              <View style={styles.footer}>
                <View style={styles.summary}>
                  ...
                  {orderDetail.promo_code_amount > 0 && (
                    <View style={styles.summaryRow}>
                      <AppText style={styles.summaryKey}>
                        ...
                        {orderDetail.auto_discount_amount ==
                          orderDetail.promo_code_amount &&
                          orderDetail.auto_discount_type == 'percentage' &&
                          '(' +
                            parseInt(orderDetail.auto_discount_discount) +
                            '%)'}
                      </AppText>
                      <AppText style={styles.summaryValue}>
                        - {orderDetail.currency_icon}{' '}
                    ...
                    <AppText style={styles.summaryValue}>
                      {orderDetail.delivery_type === 'deliver'
                        ? `${orderDetail.currency_icon} ${(
                            ...
                            orderDetail.tax_amount_with_delivery
                          ).toFixed(2)}`
                        : orderDetail.promo_code_amount > 0
                        ? `${orderDetail.currency_icon} ${(
                            +orderDetail.amount - orderDetail.promo_code_amount
                          ).toFixed(2)}`
                        : `${
                            orderDetail.currency_icon
                          } ${(+orderDetail.amount).toFixed(2)}`}
                  ...
                  <View style={styles.summaryRow}>
                    ...
                    <AppText style={styles.summaryValue}>
                      {orderDetail.currency_icon}{' '}
                      {(+orderDetail.tax_amount).toFixed(2)}
                    </AppText>
                  </View>
                  <DashedLine
                    styleContainer={{
                  ...
                  <View style={styles.summaryRow}>
                    ...
                    <AppText style={styles.summaryValue}>
                      {orderDetail.currency_icon}{' '}
                      {orderDetail.delivery_type === 'deliver'
                        ? (+orderDetail.total_amount_with_delivery).toFixed(2)
                  ...
                  <View style={styles.summaryRow}>
                    <AppText style={styles.summaryKey}>
                      Tip
                      {orderDetail.tip_type != 'fixed' &&
                        '(' + orderDetail.tip_percentage + '%)'}
                    </AppText>
                    <AppText style={styles.summaryValue}>
                      {orderDetail.currency_icon}{' '}
                      {(+orderDetail.tip_amount).toFixed(2)}
                    </AppText>
                  </View>
