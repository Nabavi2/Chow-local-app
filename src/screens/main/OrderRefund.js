...
export const OrderRefundScreen = ({ navigation }) => {
  ...
  const total_amount_formatted = useMemo(() => navigation.getParam('total_amount_formatted'), []);
        ...
        message: (
          <>               
            ...
            <AppText
              ...
              A refund of {currency_icon}{amount} has been sent to {customer_name ==' ' ? 'customer' : customer_name} (order #{order_id})
            </AppText>              
            <Button
              type="white"
              fullWidth
  ...
  const sendPartialRefund = useCallback((refundAmount) => {
    if(refundAmount > total_amount) {
      dispatch(showNotification({ type: 'error', message: "Please enter an amount up to "+total_amount_formatted }));
      return;
    }
    setLoading(true);
    console.log("RERRRRFund SENTTTTTTTTTTTTTTTTTTTTTTTTT", order_id);
    const formData = new FormData();
    formData.append('order_id', order_id);
    ...
    <Screen isLoading={isLoading}>
      <View style={styles.container}>
        <AppText style={{ fontSize: 18, textAlign: 'center'}}>
          {step == 1 ? 'Would you like to issue a full or partial refund?' : 'Enter an amount up to '+total_amount_formatted}
        </AppText>
        {step == 1 ? 
        <View style={GlobalStyles.formControl}>
          <Button
            type="bordered-dark"
            onClick={() => {sendFullRefund();}}>
            FULL REFUND
          </Button>
