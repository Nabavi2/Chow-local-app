...
import { showNotification, setTerritory, updateStatus, isUpdateStatus  } from '~/store/actions';
  ...
  const cancelOrder = useCallback(() => {
    ...
    fetchAPI('/order/cancel', {
      ...
      .then((res) => {       
        ...
        dispatch(showNotification({ type: 'success', message:"Order Cancelled Successfully" }));
      })
      .catch((err) =>{
        dispatch(showNotification({ type: 'error', message: err.message }));
      })
  ...
  return (
    <Screen isLoading={isLoading}>  
        <View style={styles.container} >
          {orderDetail && orderDetail.user.is_guest == false && <View>
          <Button
            type="bordered-dark"
            style={GlobalStyles.formControl}
            onClick={() => {
              Linking.canOpenURL(`tel:${orderDetail.user.phone}`).then((supported) => {
                if (supported) {
          ...
          <Button
            ...
            onClick={() => {
              // cancelOrder();    
              NavigationService.navigate('OrderRefund', {currency: orderDetail.territory.currency.icon,
                total_amount_formatted: orderDetail.allowed_refund_amount_formatted,
                total_amount: orderDetail.allowed_refund_amount,
                order_id: orderDetail.order_id,
                currency_icon: orderDetail.currency_icon,
                customer_name: orderDetail.customer_first_name + ' ' + orderDetail.customer_last_name,
...
OrderOptionsScreen.navigationOptions = ({ navigation }) =>
  MainNavigationOptions({
    navigation,
    options: {
      headerTitle: `#${navigation.getParam('orderId') ? navigation.getParam('orderId') : ""} ${navigation.getParam('orderStatus') ? navigation.getParam('orderStatus') : ""}`,
