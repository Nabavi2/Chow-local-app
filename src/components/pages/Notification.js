...
export const Notification = ({}) => {
  ...
  const containerStyle = useMemo(
    () =>(notification.type === 'Alert' ? styles.alert_screen :  (notification.type === 'success' ? styles.fullScreen : 
          (notification.type === 'fullScreen' || notification.type == 'error' || notification.type == 'error_card'? styles.fullScreen : styles.danger) )
    ),
    [notification],
  );
    ...
    <Modal
      ...
      <View
        style={[
          ...
          {/* {["Alert"].indexOf(notification.type) > -1 &&<><View style={styles.fullScreenWrapper_alert}>
            <AppText style={styles.alert_title}>New Notification</AppText>
            <DashedLine></DashedLine>
            <AppText style={styles.alert_message}>{'\n' + notification.message}</AppText>
          </View></>} */}
        {["fullScreen","error","success","error_card"].indexOf(notification.type) > -1 ? 

        <> 
        <View style={styles.fullScreenWrapper}>
          {notification.message && (
            ["error","success","error_card"].indexOf(notification.type) > -1
            ? 
            <>
            ...
            <Button
              ...
              {notification.buttonText ? notification.buttonText : (notification.type == 'success' ? 'Continue' : notification.type == 'error_card' ? 'Update Credit Card' : 'Try Again')}
            </Button>
            </>
            :            
            notification.message
